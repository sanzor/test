...
const mapNoticeRequestToServicePayload = (payload) => {
  const { description, content, ...rest } = payload;
  return {
    ...rest,
...
const handleAddNotice = asyncHandler(async (req, res) => {
  ...
  const payload = mapNoticeRequestToServicePayload(req.body);
  const result = await addNotice({ ...payload, authorId });
...
const handleUpdateNotice = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const payload = mapNoticeRequestToServicePayload(req.body);
  const result = await updateNotice({ ...payload, id });
