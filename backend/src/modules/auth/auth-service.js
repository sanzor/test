...
const login = async (username, passwordFromUser) => {
  ...
  try {
    ...
    await upsertRefreshSession({ userId, refreshToken });
...
const logout = async (refreshToken) => {
  ...
  await invalidateRefreshSessionByToken(refreshToken);
...
const getNewAccessAndCsrfToken = async (refreshToken) => {
  ...
  try {
    ...
    let user = null;
    const cachedUserId = await getUserIdByRefreshTokenFromCache(refreshToken);
    if (cachedUserId) {
      user = await findUserById(cachedUserId);
    }

    if (!user) {
      user = await findUserByRefreshToken(refreshToken);
      if (user?.id) {
        await upsertRefreshSession({ userId: user.id, refreshToken });
      }
    }
